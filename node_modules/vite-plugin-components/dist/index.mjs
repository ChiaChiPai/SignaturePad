...
function resolveOptions(options, viteConfig) {
  ...
  resolved.globs = resolved.dirs.map((i) => resolved.deep ? (0, import_utils2.slash)(join2(i, `**/*.${extsGlob}`)) : (0, import_utils2.slash)(join2(i, `*.${extsGlob}`)));
...
async function generateDeclaration(ctx, root, filepath) {
  const lines = Object.values(__spreadValues(__spreadValues({}, ctx.componentNameMap), ctx.componentCustomMap)).map(({path, name, importName}) => {
    if (!name)
      return void 0;
    const related = (0, import_utils2.slash)(path).startsWith("/") ? `./${relative(dirname2(filepath), resolve2(root, path.slice(1)))}` : path;
    if (!/^\w+$/.test(name))
      name = `'${name}'`;
...
var Context = class {
  ...
  onUpdate(path) {
    ...
    Object.entries(this._componentUsageMap).forEach(([key, values]) => {
      if (values.has(name)) {
        const r = `/${(0, import_utils2.slash)(relative2(this.viteConfig.root, key))}`;
  ...
  relative(path) {
    ...
    return (0, import_utils2.slash)(relative2(this.root, path));
  }
  searchGlob() {
    if (this._searched)
      return;
